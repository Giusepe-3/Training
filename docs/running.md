...
**Core read:** under-endured, not slow (5K predicts 1:59 HM; ran 2:09). Volume closes the gap. **Tibia gates everything — distance set by bone, not breath.** This plan dies from dark weeks, not slow days.
...
| Wks | Phase | Running | Intensity |
|-----|-------|---------|-----------|
| 1–4 | Re-entry / base | ALL easy Z2, bone-gated ramp | none (strides intro W3) |
| 5–8 | Build | long run grows, threshold enters | pyramidal: 1 threshold day/wk |
| 9–10 | Peak | biggest longs + race-pace specificity | polarized: RP intervals + RP long-run finishes |
| 11–12 | Taper | −25% then −55% volume, intensity & frequency KEPT | short RP touches only |
...
**Z2 = 130–161 (user-confirmed). Working band 145–155, ceiling 160.** Easy/long runs live here. Walk breaks free anytime HR hits ceiling.

| Type | Pace | HR | RPE |
|------|------|-----|-----|
| Easy / long | by HR, ~6:50–7:30 | 145–155, ≤160 | 3–5 |
| Threshold | 5:25–5:40 | ~high-160s–178 | 6–7 |
| Race pace | 5:40 | whatever it costs (drift OK late) | 6→8 |
| Strides | fast-relaxed 20 s | n/a | quick, not sprint |
...
**In-run (table above):** longs ≥90 min fuel on schedule; already validated 47 g/h GI-clean (6/22). >60 g/h REQUIRES dual-source (2:1 glu:fru — Maurten/SiS Beta Fuel/etc). Advance only if prior long GI-clean; not clean → hold dose, repeat. Sodium on every >90′ run. **No fasted long runs — carbs around every run (bone + LEA evidence).**
...
| Wk | Test | Read |
|----|------|------|
| W4 Sat | 8k @ HR ≤150, same conditions as 7/18 | expect ≥15 s/km faster than 7/18. Flat = fine (base building). Slower = flag, audit sleep/cut |
| W8 Sat | 2k WU + **6k tempo** + 2k CD | 6k avg **≤5:35 → sub-2:50 GREEN** · 5:36–5:50 → keep 2:50, open at 5:45s · **>5:50 → race the floor (6:00/km even)** |
| W10 Sun | 24k full rehearsal — kit, gels, pacing, last 5k @ 5:40 | finish RPE ≤7 with RP segment held = green light |

---

## Race plan — Sun Oct 11

- **0–5k: 5:45–5:50, HR ≤170.** Feels too easy. That's correct. No banking time.
- **5–25k: 5:38–5:40 locked.** Gel every 20′ from k4. HR drifts — let it.
- **25–30k: empty it.** Anything ≤5:40 closes under 2:50 (~2:49:2x on this shape).
- Floor plan (if W8 says so): 5:58–6:00 even = sub-3:00, still faster pace than Madrid over +9 km.
- Week-of: no legs W12, Thu upper very light, carbs up final 48 h, kit + fuel laid out from W10 rehearsal.
...
Unchanged: two-touch AM/PM, run days → `templates/daily.md`, data → xlsx, narrative → `session_log.md`. Weekly km + long-run distance + cadence avg + fuel g/h tracked every week; Sunday review reports ACWR vs table.
