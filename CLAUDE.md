...
- **🚲 BIKE COMMUTE = TIER 1 LOAD — disclosed 2026-09-07. 24 km every uni day (12 each way), uni = Mon/Tue/Wed/Thu → 96 km/wk, ~4–5 h.** Was unmodeled; this is the waiter-job error repeating, so it is logged (`Bike km:` in every PM block), never filed as transport. **Zero bone cost — non-impact, does not spend the tibia budget**, and it partly explains why the engine keeps auditing ahead of the chassis (the dark month cost ~1 bpm because cycling held the aerobic system). **Cost is quads/hip flexors/systemic recovery, Mon–Thu, exactly where the loaded days sit.** Rules: commute rides **easy Z1**, especially before a PM run · **do NOT add running km because of the bike** (bone sets running distance) · **do NOT cut it either** (free aerobic volume, and this is a finishing problem) · **Sat/Sun are bike-free — the long run stays protected, keep it that way.** ⚠️ **kcal corrected: ~2400 was stale — it subtracted the job's walking and never added the bike. Uni days ~2650–2700 · non-uni ~2400 · Sunday long ~2700 · protein 155–165 g.** Explains BW 75.85 flat 9/5–9/7 on +937/+93 kcal days. Flag: quad/hip-flexor DOMS ≥2 on a run morning, or Tue/Thu pace drifting at held HR → bike is eating the runs. Detail → `logs/session_log.md` § "BIKE COMMUTE".
- **🏞️ TRAINING VENUE IS A CALISTHENICS PARK — bars only. No dumbbells, no bands, no rings, NO ASSISTANCE OF ANY KIND (confirmed 2026-09-07).** Never prescribe assisted/banded/loaded variations. **B2 slot settled: a SECOND PUSH-UP BLOCK, 3×6–10 @ RPE 7 at one bar setting EASIER than A1** (seed setting 3) — it runs on pre-fatigued triceps and advances only when A1 advances. ⛔ **Rejected and closed: pike push-ups, dips of every kind, close-grip/diamond push-ups.** Do not re-propose them. Push/pull set balance stays **7/7**. ⚠️ Consequence: no overhead/vertical pressing in the block — irrelevant to Oct 11, revisit post-race. Detail → `workouts/cali_fullbody.md`.
- **📌 GATED GOAL-PACE ADDENDUM (2026-09-06) — ⚠️ BENCHMARK DEFERRED Fri Sep 18 → FRI SEP 25 (2026-09-15, illness). Does NOT open before Fri Sep 25; nothing above HR 150 until then. Wk C is 100% easy.** Race pace 6:20/km ≈ **HR 160–168**, and as planned he reaches Oct 11 having never run a step there (only intensity data is Madrid, all-out, HR 180). Fix = **~8 km of goal pace total, placed INSIDE the two longest runs** — no extra sessions, no extra impact days: **Fri Sep 25** (was Sep 18) 8 k benchmark (**≤5:50/km → sub-3:00 opens · 5:50–6:10 → sub-3:10 confirmed · >6:10 → race conservative**) · **Sun Sep 27** long 18 k = 15 k easy + **final 3 k @ 6:20** · **Sun Oct 4** long 22 k = 17 k easy + **final 5 k @ 6:20**, full kit/fuel rehearsal. **Any pinpoint shin pain VOIDS the whole addendum. Nothing above HR 150 before Sep 25.** Detail → `logs/session_log.md` § "ADDENDUM 2026-09-06".
- **🎵 CADENCE — METRONOME 166 ON EVERY RUN, AND THE PACE-LINKAGE IS NOT SOLVED (re-opened 2026-09-08).** Cued runs hit 166 / 166 / 167 / 167 at **7:08–7:18/km**, then **9/8 ran 8:01/km and came in at 163 — the first cued miss.** The cue was never tested below ~7:20/km; at genuinely slow easy pace the stride overrides the beat. **This is a bone lever, not cosmetic** — 163 vs 167 over a 12–22 k long run is real added impact per strike, and the tibia gates the block. ⇒ **Metronome 166 on every run including every long run. At slow paces match the beat by SHORTENING STRIDE, never by speeding up — if cadence drifts, slow the legs and hold the turnover.** ⛔ **Never re-test the uncued case; 9/2 already ran that experiment (160) and it failed.** ⚠️ **Every long run opens at ~7:50/km — exactly the pace that produced 163. Watch it on all of them.** ⚠️ **9/11 did NOT close it: cadence 166 came at 7:22/km against an open-at-7:50 prescription — 28 s/km hot, back inside the band where the cue always works. A CADENCE HIT ONLY COUNTS IF THE PACE INSTRUCTION WAS OBEYED — check pace first; under ~7:40/km the data point is void.** ⚠️ **The opening-pace correction decays after exactly one run (9/7 hot → 9/8 obeyed → 9/11 hot). RE-STATE IT ON EVERY LONG RUN; it does not stick on its own.**
- **⚠️ RPE IS A BREATH SIGNAL; DISTANCE IS SET BY BONE.** Recorded 2026-09-06 after a 10 k at RPE 4 with *"could have done 15 km chill"*. **Do not raise a long run because an easy run felt easy** — the clean shin checks are the *product* of the conservative ramp, not a licence to abandon it. When the engine is ahead, the lever is **compliance (run all 4 prescribed runs)**, never a bigger long run.
- **🤒 ILLNESS — first sick days 2026-09-12/13 (cost cali #4 + the Wk B 12 k long). NECK CHECK governs: above neck only (nose/throat/sneeze) → easy run, reduced distance, HR ceiling 140 · below neck (fever/chest/body aches/GI) → ZERO training until 24 h FULLY symptom-free, then ONE easy short run before anything loaded. Fever is a hard stop — myocarditis risk is not traded for a session. "Better than yesterday" ≠ clear.** ⚠️ **ILLNESS IS NOT DRIFT — do not treat two sick days like the Jul dark months. No make-up sessions, no clawed-back km; repeat, don't compress. Panic-compressing is the actual risk.** Re-entry = one easy SHORT run first, HR ceiling 140, distance set by bone — never a jump back onto the ramp where it stopped. Illness VOIDS the BW trend; make no kcal change on illness data. **Checkpoints are MOVEABLE, the bone is not: a benchmark run on a post-illness engine reads falsely slow and would retire a goal for the wrong reason — call it off the first clean AM, never off the calendar.**
- **Full 6-week block table, class-fitted weekly template, and Week A prescription → `logs/session_log.md` § "RE-PLAN 2026-08-31".**

---

## ⚠️ SUPERSEDED OVERRIDE — 2026-07-17: BLOCK 2 — SUB-2:50 30K (race Sun Oct 11, 2026)
...
~~**ACTIVE GOAL: 30K in sub-2:50 (5:40/km).** Floor = sub-3:00.~~ **❌ RETIRED 2026-08-31 → sub-3:10. See top override.**
...
## Weekly schedule — Block 2 ✅ LOCKED (re-cut 2026-08-31, CLASS-fitted)

**4 run / 2 cali / 3 rest.** Pegged to classes (Mon 12–17, Tue–Thu 8–12, Fri–Sun free) and the two fixed cali class times.

| Day | Class | Session | Timing |
|-----|-------|---------|--------|
| Mon | 12:00–17:00 | **Cali gym — no run** | AM, pre-class. + Tibialis Raise 3×20 |
...
| Fri | free | **Cali gym AM + Run PM** | gym class AM, run ~16:00. + Tibialis Raise. **Checkpoint slot — deferred to Fri Sep 25** |
...
Max 2 consecutive loaded days. Long run always off a full rest day. Same proven shape as the 7/27 template, re-pegged from shifts to classes.

> **DORMANT — shift-fitted template (2026-07-27), superseded 2026-08-31 when the job ended:**

<details><summary>old shift-fitted template</summary>

### ~~Weekly schedule — shift-fitted (2026-07-27)~~
...
6. **❌ DORMANT 2026-08-31 (job over) — demoted to Tier 3.** ~~Steps + shift hours — LOAD, not NEAT (2026-07-27).~~ The waiter job is ~44 km/wk walked ≈ 2.75× the running. Gates: day >22 k steps → next run easy+short · two consecutive 25 k+ days → drop the week's shortest run, keep the long. Log every PM.
...
### Nutrition baselines — ❌ CUT CLOSED 2026-08-31 · MAINTENANCE TO THE RACE

**LIVE TARGETS (2026-08-31 → Oct 11): kcal ~2400/d maintenance · protein 150–165 g (2.0–2.2 g/kg @ 75 kg) · creatine 5 g/d · carbs up on Sun long days.** BW 75.0 reached; the economy gain is banked. No deficit while ramping volume on detrained legs. Everything below is the superseded slow-cut spec, kept for the post-race resumption.

### ~~Nutrition baselines — SLOW CUT (2026-07-17, superseded 2026-08-31)~~
...
| Stretch | sub-3:00 | 6:00/km | Opens only if the 8 k benchmark clears — **deferred to Fri Sep 25** (illness, 2026-09-15) |
...
**Supporting targets:** BW 77.6 → **~73 kg** (0.3–0.5 kg/wk slow cut through base wks; maintenance final 3–4 wks + taper) — worth ~4–5 min over 30K, a third of the gap | **strength retained** at 2×/wk maintenance, slow-steady bump gate, no PR chasing | **cadence 165+** habitual at moderate pace, ⚠️ still unsolved below ~7:20/km (163 on 9/8 — see the cadence bullet in the active override) | **zero dark weeks** — volume consistency is the whole target.
